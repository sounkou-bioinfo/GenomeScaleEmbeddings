...
# Plot: density contours for all, points for coffee SNPs only

ggplot(plotDf, aes(x = PC1, y = PC2)) +
  stat_density_2d(aes(fill = ..level..), geom = "polygon", color = NA, alpha = 0.3) +
  geom_point(data = subset(plotDf, !is.na(gen)), aes(color = gen), alpha = 0.7) +
  labs(title = "PC1 vs PC2: coffee SNPs as points, others as density contours") +
  theme_minimal() +
  guides(fill = "none", color = guide_legend(title = "Gene annotation"))
